import { Diagnostic, DiagnosticSeverity, DocumentSemanticTokensProvider, Position, ProviderResult, Range, SemanticTokens, SemanticTokensBuilder, TextDocument } from "vscode";
import { aliasData, diagnostics, functionData, legend, log, parameterData, variableData } from "./storage";
import { ParseError, ParseText } from 'parser-combinators';
...
export const tokenProvider: DocumentSemanticTokensProvider = {
	...
	//   try {
	// 	const parseResult = ParseText(fullText, languageParser);
	//   } catch (p: unknown) {
	// 	if (p instanceof ParseError) {
	// 		log.appendLine(p.getPrettyErrorMessage())
	// 		const position = document.positionAt(p.index);
	// 		diagnostics.set(document.uri, [new Diagnostic(
	...
	//   }
      ...
      for (const func of functionParams) {
          if (func.text != null) {
              ...
              const functionUsage = getFunctionUsages(document, document.getText(func.name));
